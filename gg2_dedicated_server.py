...
class Character:
    ...
    def good_move_contact_solid(self, arg0, arg1):
        ...
        while total_moved < max_distance and i > 0:
            ...
            if place_free(self, new_x, new_y, loaded_map.wm_collision_rects):
                total_moved = total_moved + math.dist([self.x, self.y], [new_x, new_y])
                self.x = new_x
                self.y = new_y
                if i < MAX_I:
                    break
    ...
    def character_hit_obstacle(self):
        self.hspeed = self.hspeed * delta_factor
        self.vspeed = self.vspeed * delta_factor
        ...
        while (abs(hleft) > 0.1 or abs(vleft) > 0.1) and stuck == 0:
            ...
            hleft = hleft - (self.x - prev_x)
            vleft = vleft - (self.y - prev_y)
    ...
    def begin_step(self):
        ...
        for x in range(frameskip):
            if not self.taunting and not self.omnomnomnom:
                if num_to_bool((hex_as_int(self.key_state) | hex_as_int(self.pressed_keys)) & 0x40) and self.hspeed >= -self.base_max_speed:
                    self.hspeed = self.hspeed - (self.run_power * self.control_factor * skip_delta_factor)
                    controlling = True
            ...
            if abs(self.hspeed) > self.base_max_speed * 2 or (num_to_bool((hex_as_int(self.key_state) | hex_as_int(self.pressed_keys)) & 0x60) and abs(self.hspeed) < self.base_max_speed):
                self.hspeed = self.hspeed / (self.base_friction * skip_delta_factor + (1-1*skip_delta_factor))
            else:
                self.hspeed = self.hspeed / (self.friction_factor * skip_delta_factor + (1-1*skip_delta_factor))
        ...
        if not on_ground and not stuck_in_wall:
            if rocket_jumping:
                self.applied_gravity = self.applied_gravity + 0.54
            else:
                self.applied_gravity = self.applied_gravity + 0.6
    ...
    def normal_step(self):
        ...
        if self.spin_jumping and place_free(self, self.x, self.y - _gravity, loaded_map.wm_collision_rects) and (place_free(self, self.x, self.y +1, loaded_map.wm_collision_rects) or self.vspeed < 0):
            self.applied_gravity = self.applied_gravity - _gravity
        else:
            self.spin_jumping = False

        # Gravity
        self.vspeed = self.vspeed + (self.applied_gravity*delta_factor/2)
        if self.vspeed > 10:
            self.vspeed = 10
        ...
        if do_hit:
            ...
            self.x = self.x + (self.hspeed * delta_factor)
            self.y = self.y + (self.vspeed * delta_factor)
